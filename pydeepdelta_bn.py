...
class DeepDelta(object):
    ...
    def __init__(self, params, cost, reg_lambda, reg_lambda_val, yhat, X, y, 
                 ...
                 dropout_rate=None, dropout_rate_val=None, batch_size_F=None, logits=False, is_training=None, is_training_val=False):
        ...
        self.is_training_val = is_training_val
        self.yhat = yhat
        self.X = X
        self.y = y
    ...
    def _parse_params(self, params, layer):
        ...
        try:
            dl = int(re.sub(r'\D\D', "", self.layers[layer]))
        ...
        for b in range(B):
            Hv = Hv + self.tfsession.run(self._Hv_op, 
                                       feed_dict={self.X:self.X_train[b*Bs:(b+1)*Bs], 
                                                  ...
                                                  self._v:np.squeeze(v), self.is_training: self.is_training_val})
        Hv = Hv / B
        self._b = self._b + 1
    ...
    def eval_F_op(self, X, progress=True, is_training=False):
        """
    ...
    def eval_Hv_op(self, v, progress=True):
        """
        ...
        if self._v is None:
            self._v = tf.placeholder(dtype='float32', shape=(self.layers_P))
    ...
    def compute_eig_H(self, progress=True):
        ...
        Lambda,Q = eigsh(H, k=self.K, v0=v0, which='LA', tol=10**-4)
    ...
    def get_J_op(self):
        ...
        ex_X = tf.split(self.X, self.batch_size_G)
        ex_y = tf.split(self.y, self.batch_size_G)    
        ex_yhat_logits = [self._model_fun(_X, _params, True) \
                          for _X, _params in zip(ex_X, 
                                                 ex_params)]
        ...
        for b in self._progress(range(B), progress):
            J[self.batch_size_G*(b%N):self.batch_size_G*(b%N+1),:] = self.tfsession.run(self._J_op, 
                                                                                        feed_dict={self.X:self.X_train[b*self.batch_size_G: \
                                                                                                   ...
                                                                                                   self.y:self.y_train[b*self.batch_size_G: \
                                                                                                                       (b+1)*self.batch_size_G],
                                                                                                   self.is_training: self.is_training_val})
    ...
    def get_var_H_op(self):
        ...
        if self._F_op is None:
            self._F_op = self.get_F_op()
        if self._var_H_op is None:
            self._var_H_op = (tf.reduce_sum(tf.matmul(self._F_op, 
                                                               tf.gather(self._Q_H, 
                                                                         self._H_Kinds, 
                                                                         axis=1))**2 \
                                                     / tf.gather(self._Lambda_H,
                                                                 ...
                                                                 axis=0),
                                                     axis=1) \
                                       + self._lambda_H_gap_inv*(tf.reduce_sum(self._F_op**2, 
                                                                               axis=1) \
                                                                 - tf.reduce_sum(tf.matmul(self._F_op, 
                                                                                           tf.gather(self._Q_H, 
                                                                                                     self._H_Kinds, 
                                                                                                     axis=1))**2, 
                                                                                 axis=1))) / self.N_train 
                             
        return self._var_H_op
  

    def get_var_H_err_op(self):
        """
        ...
        if self._F_op is None:
            self._F_op = self.get_F_op()
        if self._var_H_err_op is None:
            self._var_H_err_op = self._lambda_H_gap_err_inv * (tf.reduce_sum(self._F_op**2, 
                                                                                      axis=1) \
                                                                        - tf.reduce_sum(tf.matmul(self._F_op, 
                                                                                                  tf.gather(self._Q_H, 
                                                                                                            self._H_Kinds, 
        ...
        return self._var_H_err_op
 

    def eval_var_H_op(self, X, K=None, returnError=False, is_training=False, progress=True, 
                      full_rank=True):
        """
        ...
        if self._var_H_op is None:
            self._var_H_op = self.get_var_H_op()
                
        if returnError is True and full_rank is True:
            if self._var_H_err_op is None:
                self._var_H_err_op = self.get_var_H_err_op()
            
            _lambda_H_gap_err_inv = (nLambda_H**-1 - pLambda_H**-1)/2.0
                                    
            return np.swapaxes(np.array([self.tfsession.run([self._var_H_op, 
                                                       self._var_H_err_op], 
                                                      feed_dict={self.X:[X[n]],
                                                                 self._H_Kinds:Kinds, 
                                                                 ...
                                                                 self.dropout_rate: self.dropout_rate_val,
                                                                 self.is_training: is_training})#self.is_training_val})
                                         for n in self._progress(range(N), progress)]), 0, 1)
        else:
            return np.array([self.tfsession.run(self._var_H_op, 
                                                     feed_dict={self.X:[X[n]],
                                                                self._H_Kinds:Kinds,
                                                                self._lambda_H_gap_inv:_lambda_H_gap_inv,
                                                                self.dropout_rate: self.dropout_rate_val,
